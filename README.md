...
| [`docs/handoff/`](docs/handoff/README.md) | **Three documents to send.** For the attorney and dev: the flow in one picture. For the dev: a build spec organized around the rules, with floors, data model, write path, validator, export gates, tests and the demo case as first integration test. For the attorney and ethics counsel: supervision by default, in pictures. |
| [`docs/demo-case/`](docs/demo-case/README.md) | **One demonstrated case.** Thirty synthetic days at the Level 2 default: the direction letter and prompt library, 107 log rows, the dual-dated source-linked timeline with per-entry California pathway candidates, nine flags with dispositions, the supervision record as it would export, a load file for the proof of concept, and a reviewer packet for a PI attorney and an adversarial litigation reviewer. |
...
| [`docs/research/competitive-watch-2026-09-hona.md`](docs/research/competitive-watch-2026-09-hona.md) | Hona's "Lia" (Sept 2026): the first funded competitor with a conversational SMS agent collecting symptoms after treatment visits. What overlaps, what does not, tripwire status, threat assessment. Supersedes the Hona lines in the PRD, the August scan and OVERVIEW. |
